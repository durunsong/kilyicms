...
14. 后台页面加载动画全局封装，骨架屏，进度条，请求loading封装，
15. 必要组件全局挂载 🎈
16. 多语言管理，多语言切换组件，批量翻译程序编写 🎈
17. 公用方法hooks封装 🎈
18. 前端路由分离
19. 自定义指令全局封装
20. 前端代码自动化Jenkins部署或通过nodejs命令行部署
21. 三种验证码校验(前端组件方式、前端后端数字运算或图形验证码校验、前端后端cf浏览器指纹自动人机校验)
22. 大文件分片上传(单独上传、富文本上传)
23. 构建微前端架构，动态添加路由，动态添加子系统，处理好css沙箱
