<template>
  <div :class="layoutClasses" class="app-wrapper">
    <!-- mobile 端侧边栏遮罩层 -->
    <div v-if="layoutClasses.mobile && layoutClasses.openSidebar" class="drawer-bg" @click="handleClickOutside"></div>
    <!-- 左侧边栏 -->
...
.sidebar-container {
  background-color: var(--kilyicms-sidebar-menu-bg-color);
...
.main-container {
  ...
  margin-left: var(--kilyicms-sidebar-width);
  position: relative;
}
...
.fixed-header {
  ...
  right: 0;
  z-index: 9;
  width: calc(100% - var(--kilyicms-sidebar-width));
  transition: width $transition-time;
}
...
.app-main {
  min-height: calc(100vh - var(--kilyicms-navigationbar-height));
  position: relative;
  overflow: hidden;
}
...
.mobile {
  ...
  &.hideSidebar {
    .sidebar-container {
      ...
      transform: translate3d(calc(0px - var(--kilyicms-sidebar-width)), 0, 0);
    }
  }
}
